...
writerObj = VideoWriter('OpticalFlow3_2.mp4','MPEG-4');        % Create video writer object
...
for count = frameStartTraining:frameEnd                  % The number of frames to process
    ...
    if count>=frameStart
        ...
        subplot(2,2,1)
        imshow(frame), axis on, title(['original frame ',num2str(count)],'Color','blue'); % Plot on top of the current frame 

        subplot(2,2,2)
        imshow(frame), axis on, title(['Triangulated frame with MNC optical flow',num2str(count)],'Color','blue');
            ...
            for i = 1: size(blbCentroids,1) % Go through blobs one by one
                ...
                for ii = 1: size(MNCLoc,1)
                    ...
                    for i3 = 1:size(MNCR,1)
                        ...
                        %patch(x,y,faceclr,'FaceAlpha',faceopac,'Marker',vrtxmrkr,'MarkerFaceColor',vrtxmrkrclr ...
                        %    ,'MarkerSize',vrtxmrkrsize,'MarkerEdgeColor',mrkredgeclr,'EdgeColor',edgeclr,'LineWidth',edgewidth);
                        %for j = 1:length(y)
                        %    if (floor(x(j))<frameSize(2) && floor(y(j))<frameSize(1))
                        %         text(x(j),y(j),['\leftarrow ',num2str(frame_hue(floor(y(j)),floor(x(j))))],'Color','blue','FontSize',12);
                        %    end
                        %end
                        %grad =zeros(3,1);
                        %div =zeros(3,1);
                ...
                neg=1;
                pos=1;
                %while(xx<max_x && xx>min_x && yy<max_y && yy>min_y)
                for i=1:150
                    if(curl<0)
                        pos = 1;
                        xx = xx+x_cw(neg);
                    ...
                    %if (yy+1<frameSize(1) && xx+1< frameSize(2))
                        curl = double(frame(yy+1,xx)) - double(frame(yy,xx+1));
                    %end
        ...
        subplot(2,2,4)
        plot(unique(C),'o','MarkerFaceColor','r', 'MarkerEdgeColor','b'), axis on, title(['Sorted curl of MNC vertices ',num2str(count)],'Color','blue');
        xlabel('vertices'), ylabel('lx')
        end
        hold off
